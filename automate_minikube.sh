...
hiptest-publisher --config-file test/hiptest-publisher.conf --test-run-id 546821 --only=features
...
hiptest-publisher --config-file test/hiptest-publisher.conf --push test/results.json  --test-run-id 546821 --push-format cucumber-json
end=$(date +%s)
echo $(($end-$start))
